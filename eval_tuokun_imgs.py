...
import eval_utils_tuokun as eval_utils_tuokun
import argparse
...
replace = ['input_fc_dir', 'input_att_dir', 'input_box_dir', 'input_attr_dir', 'input_word_dir', 'input_label_h5', 'input_json', 'batch_size', 'id']
ignore = ['start_from']

for k in vars(infos['opt']).keys():
    if k in replace:
        if k == 'input_fc_dir':              ######################################
            setattr(opt, k, './data/tuokun/tuokun_fc')
        elif k == 'input_att_dir':
            setattr(opt, k, './data/tuokun/tuokun_att')
        elif k == 'input_box_dir':
            setattr(opt, k, './data/tuokun/tuokun_box')
        elif k == 'input_attr_dir':
            setattr(opt, k, './data/tuokun/tuokun_attr_name/num')
        elif k == 'input_word_dir':
            setattr(opt, k, './data/tuokun/tuokun_object_name/num')

        elif k == 'input_json':
            setattr(opt, k, './data/tuokun/tuokun_test_talk.json')
        elif k == 'input_label_h5':
            setattr(opt, k, './data/tuokun/tuokun_talk.h5')
        else:
...
split_predictions = eval_utils_tuokun.eval_split(model, crit, loader,
        vars(opt))
