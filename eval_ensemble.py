...
# Input paths
parser.add_argument('--ids', nargs='+', required=True, default='', help='id of the models to ensemble')
...
for id in opt.ids:
    if '-' in id:
        ...
        app = ''
    model_infos.append(utils.pickle_load(open('log/log_%s/infos_%s%s.pkl' %(id, id, app), 'rb')))
    model_paths.append('log/log_%s/model%s.pth' %(id,app))
...
if opt.dump_json == 1:
    # dump the json
    json.dump(split_predictions, open(opt.save_path_seq, 'w'))
