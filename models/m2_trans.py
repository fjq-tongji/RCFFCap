...
class Encoder(nn.Module):
    ...
    def forward(self, fc_feats, att, word_feats, attr_feats, seg_feats, boxes_feats, mask):  ## fc(5,512)
        ...
        for layer in self.layers:
            ...
            x = self.norm(x)
            outs.append(x.unsqueeze(1))  ##(5,1,B,512)
...
class Decoder(nn.Module):
    ...
    def forward(self, x, memory, src_mask, tgt_mask):    ## memory = (5,4,B,512)
        ...
        return self.norm(x), attn_t, attn_c
...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, size, self_attn, src_attn, feed_forward, dropout):
        ...
        self.enc_att = MultiHeadedAttention(h=8, d_model=512)

        self.fc_alpha1 = nn.Linear(512 + 512, 512)
        self.fc_alpha2 = nn.Linear(512 + 512, 512)
        self.fc_alpha3 = nn.Linear(512 + 512, 512)
        self.fc_alpha4 = nn.Linear(512 + 512, 512)

    def forward(self, x, enc_output, src_mask, tgt_mask):        ## memory = (5,4,B,512)
        "Follow Figure 1 (right) for connections."
        x = self.sublayer[0](x, lambda x: self.self_attn(x, x, x, tgt_mask))  ### masked MSA

        enc_att1 = self.enc_att(x, enc_output[:, 0], enc_output[:, 0], src_mask)
        enc_att2 = self.enc_att(x, enc_output[:, 1], enc_output[:, 1], src_mask)
        #enc_att3 = self.enc_att(x, enc_output[:, 2], enc_output[:, 2], src_mask)
        #enc_att4 = self.enc_att(x, enc_output[:, 3], enc_output[:, 3], src_mask)

        alpha1 = torch.sigmoid(self.fc_alpha1(torch.cat([x, enc_att1], -1)))
        alpha2 = torch.sigmoid(self.fc_alpha2(torch.cat([x, enc_att2], -1)))
        #alpha3 = torch.sigmoid(self.fc_alpha3(torch.cat([x, enc_att3], -1)))
        #alpha4 = torch.sigmoid(self.fc_alpha4(torch.cat([x, enc_att4], -1)))

        enc_att = (enc_att1 * alpha1 + enc_att2 * alpha2) / np.sqrt(2)
        ######

        #x = self.sublayer[1](x, lambda x: self.src_attn(x, m, m, src_mask))  ### cross MSA
        return self.sublayer[2](enc_att, self.feed_forward)
...
class m2TransformerModel(AttModel):
    ...
    def _prepare_feature_forward(self, fc_feats, att_feats, word_feats, attr_feats, seg_feats, boxes_feats, att_masks=None,
                                 seq=None):
