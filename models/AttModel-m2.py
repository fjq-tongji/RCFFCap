...
class AttModel(CaptionModel):
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks):
        #print('***********************')
    ...
    def _sample_beam(self, fc_feats, att_feats, word_feats, attr_feats, boxes_feats, att_masks=None, opt={}):
        beam_size = opt.get('beam_size', 10)
        batch_size = fc_feats.size(0)

        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks = self._prepare_feature(fc_feats, att_feats, word_feats, attr_feats, boxes_feats, att_masks)
        ...
        for k in range(batch_size):
            ...
            tmp_p_att_feats = []
            for i in range(6):
                tmp_p_att_feats_ = pp_att_feats[i][k:k + 1].expand(*((beam_size,) + pp_att_feats[i].size()[1:])).contiguous()
                tmp_p_att_feats.append(tmp_p_att_feats_)

            tmp_att_masks = p_att_masks[k:k+1].expand(*((beam_size,)+p_att_masks.size()[1:])).contiguous() if att_masks is not None else None


            #print('^^^^^^^^^^^^^^^^^^^^^^^^^')
            #print(tmp_p_att_feats[0].shape)
            #print(tmp_p_att_feats[1].shape)
            #print(tmp_p_att_feats[2].shape)
            #print(tmp_p_att_feats[3].shape)
            #print(tmp_att_masks.shape)
            ...
            self.done_beams[k] = self.beam_search(state, logprobs, tmp_fc_feats, tmp_att_feats, tmp_p_att_feats[-1], tmp_att_masks, opt=opt)
            seq[:, k] = self.done_beams[k][0]['seq'] # the first beam has highest cumulative score
            seqLogprobs[:, k] = self.done_beams[k][0]['logps']
        # return the samples and their log likelihoods
        return seq.transpose(0, 1), seqLogprobs.transpose(0, 1)

    def _sample(self, fc_feats, att_feats, word_feats, attr_feats, boxes_feats, att_masks=None, opt={}):
        ...
        if beam_size > 1:
            return self._sample_beam(fc_feats, att_feats, word_feats, attr_feats, boxes_feats, att_masks, opt)
