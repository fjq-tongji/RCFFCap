...
class AttModel(CaptionModel):
    ...
    def _forward(self, fc_feats, att_feats, word_feats, attr_feats, seg_feats, boxes_feats, seq, att_masks):
        #print('***********************')
        ...
        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks = self._prepare_feature(fc_feats, att_feats, word_feats, attr_feats, seg_feats, boxes_feats, att_masks)
        #print('############pp_att_feats########')
    ...
    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, state):
        ...
        output, state, attn_t, attn_c = self.core(xt, fc_feats, att_feats, p_att_feats, att_masks, state)
        logprobs = F.log_softmax(self.logit(output), dim=1)
        #print('####################')
        #print(logprobs.shape)

        return logprobs, state, attn_t, attn_c

    def _sample_beam(self, fc_feats, att_feats, word_feats, attr_feats, seg_feats, boxes_feats, att_masks=None, opt={}):
        beam_size = opt.get('beam_size', 10)
        batch_size = fc_feats.size(0)
        #print(batch_size)


        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, attn_e = \
            self._prepare_feature(fc_feats, att_feats, word_feats, attr_feats, seg_feats, boxes_feats, att_masks)
        #print(attn_e.shape)

        #pp_att_feats == memory (encoder output)
        ...
        for k in range(batch_size):
            ...
            for t in range(1):
                ...
                logprobs, state, attn_t, attn_c = self.get_logprobs_state(it, tmp_fc_feats, tmp_att_feats, tmp_p_att_feats, tmp_att_masks, state)

            self.done_beams[k], attn_t, attn_c = self.beam_search(state, logprobs, tmp_fc_feats, tmp_att_feats, tmp_p_att_feats, tmp_att_masks, opt=opt)
            print('1111111')
            print(self.done_beams[k])
            #print(attn_c.shape)
        ...
        return seq.transpose(0, 1), seqLogprobs.transpose(0, 1), pp_att_feats, attn_e, attn_t,attn_c

    def _sample(self, fc_feats, att_feats, word_feats, attr_feats, seg_feats, boxes_feats, att_masks=None, opt={}):
        ...
        if beam_size > 1:
            return self._sample_beam(fc_feats, att_feats, word_feats, attr_feats, seg_feats, boxes_feats, att_masks, opt)

        batch_size = fc_feats.size(0)
        state = self.init_hidden(batch_size)

        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, attn = self._prepare_feature(fc_feats, att_feats, word_feats, attr_feats, seg_feats, boxes_feats, att_masks)
        ...
        for t in range(self.seq_length + 1):
            ...
            logprobs, state, attn = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)
