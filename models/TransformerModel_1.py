...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, fc_feats, src, word_feats, attr_feats, seg_feats, boxes_feats, tgt, src_mask, tgt_mask):
        ...
        x, attn_e = self.encode(fc_feats, src, word_feats, attr_feats, seg_feats, boxes_feats, src_mask)
        x, attn_t, attn_c = self.decode(x, src_mask, tgt, tgt_mask)
        return x, attn_e, attn_t, attn_c
...
class Encoder(nn.Module):
    ...
    def forward(self, fc_feats, att, word_feats, attr_feats, seg_feats, boxes_feats, mask):     ## fc(5,512)
        ...
        #attr_word = torch.cat([att, word_feats, attr_feats, seg_feats], dim=1)  ## (5,B1+B2+B3,512)
        ...
        #x0 = self.msa_0(x0, x0, x0, mask)
        #x = self.dropout_0(x0) + x_yuan0

        # x = att
        #c_feats = torch.cat([word_feats, attr_feats, seg_feats], dim=1)  ## (5,B1+B2+B3,512)   ###kv
        #x_yuan = x
        #x = self.norm_1(x)
        #x = self.msa(x, c_feats, c_feats)
        ...
        for layer in self.layers:
            x, attn_e = layer(fc_feats, x, word_feats, attr_feats, boxes_feats, mask)  ### x -> att
            attention_1.append(attn_e)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, fc_feats, att, word_feats, attr_feats, boxes_feats, mask):
        ...
        x_yuan = x
        x = self.norm_1(x)
        x, attn_e = self.msa(x, x, x, mask)    ##attn
        x = self.dropout_1(x) + x_yuan

        x_yuan_2 = x
        x = self.norm_2(x)
        x = self.ffn(x)
        x = self.dropout_2(x) + x_yuan_2
...
class Decoder(nn.Module):
    ...
    def forward(self, x, memory, src_mask, tgt_mask):
        ...
        attn_cross = []
        for layer in self.layers:
            x, attn_t, attn_c = layer(x, memory, src_mask, tgt_mask)
            attn_text.append(attn_t)
            attn_cross.append(attn_c)

        return self.norm(x), attn_text[-1], attn_cross[-1]
...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, size, self_attn, src_attn, feed_forward, dropout):
        ...
        self.msa_1 = MultiHeadedAttention(8, 512)
        self.msa_2 = MultiHeadedAttention(8, 512)
        self.ffn = PositionwiseFeedForward(512, 2048)
        self.norm_1 = LayerNorm(size)
        self.dropout_1 = nn.Dropout(dropout)
        self.norm_2 = LayerNorm(size)
    ...
    def forward(self, x, memory, src_mask, tgt_mask):
        ...
        x_yuan = x
        x = self.norm_1(x)
        x, attn_text = self.msa_1(x, x, x, tgt_mask)  ##attn_text
        x = self.dropout_1(x) + x_yuan

        x_yuan_2 = x
        x = self.norm_2(x)
        x, attn_c = self.msa_2(x, m, m, src_mask)  ##attn_2
        x = self.dropout_2(x) + x_yuan_2

        x_yuan_3 = x
        x = self.norm_3(x)
        x = self.ffn(x)
        x = self.dropout_3(x) + x_yuan_3

        return x, attn_text, attn_c
...
class TransformerModel_1(AttModel):
    ...
    def _prepare_feature(self, fc_feats, att_feats, word_feats, attr_feats, seg_feats, boxes_feats, att_masks):
        ...
        #print(att_feats.shape)
        memory, attn_e = self.model.encode(fc_feats, att_feats, word_feats, attr_feats, seg_feats, boxes_feats, att_masks)

        return fc_feats[...,:0], att_feats[...,:0], memory, att_masks, attn_e
    ...
    def core(self, it, fc_feats_ph, att_feats_ph, memory, mask, state):
        ...
        out, attn_t, attn_c = self.model.decode(memory, mask,
                               ys, 
                               subsequent_mask(ys.size(1))
                                        .to(memory.device))
        print(out.shape)
        return out[:, -1], [ys.unsqueeze(0)], attn_t, attn_c
