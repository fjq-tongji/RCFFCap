...
def attention_d(query, key1, key2, key3, value1, value2, value3, mask=None, dropout=None):
    ...
    scores2 = torch.matmul(query, key2.transpose(-2, -1)) \
        / math.sqrt(d_k)
    scores3 = torch.matmul(query, key3.transpose(-2, -1)) \
        / math.sqrt(d_k)
    if mask is not None:
        scores1 = scores1.masked_fill(mask == 0, -1e9)
        scores2 = scores2.masked_fill(mask == 0, -1e9)
        scores3 = scores3.masked_fill(mask == 0, -1e9)
    p_attn1 = F.softmax(scores1, dim=-1)
    p_attn2 = F.softmax(scores2, dim=-1)
    p_attn3 = F.softmax(scores3, dim=-1)
    if dropout is not None:
        p_attn1 = dropout(p_attn1)
        p_attn2 = dropout(p_attn2)
        p_attn3 = dropout(p_attn3)
    v1 = torch.matmul(p_attn1, value1)
    v2 = torch.matmul(p_attn2, value2)
    v3 = torch.matmul(p_attn3, value3)
    return (v1 + v2 + v3) / 3, p_attn1, p_attn2, p_attn3

def attention(query, key_p, key_n, key_c, value_p, value_n, value_c, mask=None, dropout=None):
    "Compute 'Scaled Dot Product Attention'"
    d_k = query.size(-1)

    scores1 = torch.matmul(query, key_p.transpose(-2, -1)) \
        / math.sqrt(d_k)
    scores2 = torch.matmul(query, key_n.transpose(-2, -1)) \
        / math.sqrt(d_k)
    scores3 = torch.matmul(query, key_c.transpose(-2, -1)) \
        / math.sqrt(d_k)
    ...
    v1 = torch.matmul(p_attn1, value_p)
    v2 = torch.matmul(p_attn2, value_n)
    v3 = torch.matmul(p_attn3, value_c)
    return v1 + v2 + v3, p_attn1, p_attn2, p_attn3
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1):
        ...
        self.q_linears = nn.Linear(d_model, d_model)
        self.k_linears = clones(nn.Linear(d_model, d_model), 3)  # (parent,neighbor,child)
        self.v_linears = clones(nn.Linear(d_model, d_model), 3)
    ...
    def forward(self, query, key, value, mask=None):
        ...
        x, self.attn, p_attn2, p_attn3 = attention(query_, key_p, key_n, key_c, value_p, value_n, value_c, mask=mask, dropout=self.dropout)
...
class MultiHeadedAttention_d(nn.Module):
    def __init__(self, h, d_model, dropout=0.1):
        "Take in model size and number of heads."
        super(MultiHeadedAttention_d, self).__init__()
        assert d_model % h == 0
        # We assume d_v always equals d_k
        self.d_k = d_model // h
        self.h = h
    ...
    def forward(self, query, key, value, mask=None):
        ...
        # 2) Apply attention on all the projected vectors in batch.
        x, self.attn, p_attn2, p_attn3 = attention_d(query_, key_p, key_n, key_c, value_p, value_n, value_c, mask=mask, dropout=self.dropout)
...
class ImageTransModel(AttModel):
    ...
    def make_model(self, src_vocab, tgt_vocab, N_enc=6, N_dec=6,
        ...
        attn_d = MultiHeadedAttention_d(h, d_model)
        ff = PositionwiseFeedForward(d_model, d_ff, dropout)
        position = PositionalEncoding(d_model, dropout)
        model = EncoderDecoder(
            Encoder(EncoderLayer(d_model, c(attn), c(ff), dropout), N_enc),
            Decoder(DecoderLayer(d_model, c(attn), c(attn_d),
                                 c(ff), dropout), N_dec),
    ...
    def __init__(self, opt):
        super(ImageTransModel, self).__init__(opt)
        self.opt = opt
