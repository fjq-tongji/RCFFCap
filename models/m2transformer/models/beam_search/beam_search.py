...
class BeamSearch(object):
    ...
    def _expand_state(self, selected_beam, cur_beam_size):
        def fn(s):
            ...
            s1 = s.view(*([self.b_s, cur_beam_size] + shape[1:]))
            s1 = torch.tensor(s1, dtype=torch.int64)
            s2 = beam.expand(*([self.b_s, self.beam_size] + shape[1:]))
            s2 = torch.tensor(s2, dtype=torch.int64)
            s = torch.gather(s1, 1, s2)
    ...
    def _expand_visual(self, visual: utils.TensorOrSequence, cur_beam_size: int, selected_beam: torch.LongTensor):
        if isinstance(visual, torch.Tensor):
            ...
            visual = torch.gather(torch.tensor(visual_exp, dtype=torch.int64), 1, torch.tensor(selected_beam_exp, dtype=torch.int64)).view(visual_red_shape)
        else:
            new_visual = []
            for im in visual:
    ...
    def iter(self, t: int, visual: utils.TensorOrSequence, outputs, return_probs, **kwargs):
        ...
        self.seq_mask = torch.gather(torch.tensor(self.seq_mask,dtype=torch.int64), 1, torch.tensor(selected_beam.unsqueeze(-1), dtype=torch.int64))
        outputs = list(torch.gather(o, 1, selected_beam.unsqueeze(-1)) for o in outputs)
        outputs.append(selected_words.unsqueeze(-1))

        s3 = selected_beam.unsqueeze(-1).expand(self.b_s, self.beam_size, word_logprob.shape[-1])

        selected_beam_logprob = torch.gather(torch.tensor(word_logprob, dtype=torch.int64), 1, torch.tensor(s3, dtype=torch.int64) ) # Bxbxvocab
        
        s4 = torch.tensor(selected_words, dtype=torch.int64)
        this_word_logprob = torch.gather(torch.tensor(selected_beam_logprob, dtype=torch.int64), 2, s4.unsqueeze(-1))
        self.log_probs = list(
