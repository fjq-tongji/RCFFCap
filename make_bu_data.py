...
# output_dir
parser.add_argument('--downloaded_feats', default='data/coco/bu_data', help='downloaded feature directory')
parser.add_argument('--output_dir', default='data/coco/coco_new', help='output feature files')
...
for infile in infiles:
    ...
    with open(os.path.join(args.downloaded_feats, infile), "r+t") as tsv_in_file:  ##r+b
        reader = csv.DictReader((line.replace('\0','') for line in tsv_in_file), delimiter='\t', fieldnames = FIELDNAMES)
        for item in reader:
