...
class DataLoader(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.word_feats_loader = HybridLoader(self.opt.input_word_dir, '.npy')
        self.attr_feats_loader = HybridLoader(self.opt.input_attr_dir, '.npy')
        self.seg_feats_loader = HybridLoader(self.opt.input_seg_dir, '.npy')  ###########
        self.box_loader = HybridLoader(self.opt.input_box_dir, '.npy')        ### HybridLoader====np.load()
    ...
    def get_batch(self, split, batch_size=None):
        ...
        for i in range(batch_size):
            # fetch image
            tmp_fc, tmp_att, tmp_word_feats, tmp_attr_feats, tmp_seg_feats, tmp_boxes, tmp_seq, \
            ...
            seg_feats_batch.append(tmp_seg_feats)
        ...
        if self.use_box:
            fc_batch, att_batch, word_feats_batch, attr_feats_batch, seg_feats_batch, boxes_batch, label_batch, gts, infos = \
                zip(*sorted(zip(fc_batch, att_batch, word_feats_batch, attr_feats_batch, seg_feats_batch, boxes_batch, label_batch, gts, infos), key=lambda x: 0, reverse=True))
        else:
            fc_batch, att_batch, word_feats_batch, attr_feats_batch, seg_feats_batch, label_batch, gts, infos = \
                zip(*sorted(zip(fc_batch, att_batch, word_feats_batch, attr_feats_batch, seg_feats_batch, label_batch, gts, infos), key=lambda x: 0, reverse=True))
        ...
        max_word_len = 20 #max(_.shape[0] for _ in word_feats_batch)
        max_attr_len = 20 #max(_.shape[0] for _ in attr_feats_batch)
        max_seg_len = max(_.shape[0] for _ in seg_feats_batch)
        ...
        data['seg_feats'] = np.zeros([len(att_batch) * seq_per_img, max_seg_len], dtype='int32')
        ...
        for i in range(len(att_batch)):
            ...
            if seg_feats_batch[i].shape[0] > max_seg_len:
                data['seg_feats'][i * seq_per_img:(i + 1) * seq_per_img, :max_seg_len] = seg_feats_batch[i][:max_seg_len]
            else:
                data['seg_feats'][i * seq_per_img:(i + 1) * seq_per_img, :seg_feats_batch[i].shape[0]] = seg_feats_batch[i]
        ...
        if self.use_box:
            ...
            #for i in range(len(boxes_batch)):
             #   data['boxes_feats'][i * seq_per_img:(i + 1) * seq_per_img, :boxes_batch[i].shape[0]] = boxes_batch[i]
    ...
    def __getitem__(self, index):
        ...
        if self.use_att:
            ...
            seg_feats = self.seg_feats_loader.get(str(self.info['images'][ix]['id']))
        ...
        if self.use_box:
            return (fc_feat,
                    att_feat, word_feats, attr_feats, seg_feats, boxes_feat, seq,           ##************************************
                    ix)
        else:
            return (fc_feat,
                    att_feat, word_feats, attr_feats, seg_feats, boxes_feat, seq,  ##************************************
