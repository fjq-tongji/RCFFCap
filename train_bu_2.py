...
def train(opt):
    ...
    try:
        while True:
            ...
            start = time.time()
            tmp = [data['fc_feats'], data['att_feats'], data['word_feats'], data['attr_feats'], data['seg_feats'], data['boxes_feats'], data['labels'], data['masks'], data['att_masks']]
            ...
            model_out = dp_lw_model(fc_feats, att_feats, word_feats, attr_feats, seg_feats, boxes_feats, labels, masks, att_masks, data['gts'], torch.arange(0, len(data['gts'])), sc_flag)
            # print(attn.shape)
            # np.save(os.path.join('/home/fjq/husthuaan/log/attn_decoder/attn_decoder_4/' + 'decoder-4'),
            #         attn.item()[-1][0][0].cpu().numpy())
