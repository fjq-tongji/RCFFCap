...
class DataLoader(data.Dataset):
    ...
    def get_batch(self, split, batch_size=None):
        ...
        for i in range(batch_size):
            # fetch image
            tmp_fc, tmp_att, tmp_word_feats, tmp_attr_feats, tmp_boxes, tmp_seq, \
            ...
            #print(tmp_att.shape)
            #print(tmp_word_feats.shape)
            #print(tmp_attr_feats.shape)
        ...
        if self.use_box:
            fc_batch, att_batch, word_feats_batch, attr_feats_batch, boxes_batch, label_batch, gts, infos = \
                zip(*sorted(zip(fc_batch, att_batch, word_feats_batch, attr_feats_batch, boxes_batch, label_batch, gts, infos), key=lambda x: 0, reverse=True))
        
            #print(word_feats_batch[0])
            #print(word_feats_batch[0].shape)
        
        
        else:
            fc_batch, att_batch, word_feats_batch, attr_feats_batch, label_batch, gts, infos = \
                zip(*sorted(zip(fc_batch, att_batch, word_feats_batch, attr_feats_batch, label_batch, gts, infos), key=lambda x: 0, reverse=True))
    ...
    def __getitem__(self, index):
        ...
        if self.use_box:
            return (fc_feat,
                    att_feat, word_feats, attr_feats, boxes_feat, seq,           ##************************************
                    ix)
        else:
            return (fc_feat,
                    att_feat, word_feats, attr_feats, seq,  ##************************************
                    ix)
